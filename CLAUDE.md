...
- **Secrets stay out.** No API keys, tokens, or credentials in source or history. The Hypixel key (and any other) lives host-side behind the `HypixelStatsSource` seam. Never commit `.idea/`, `*.iml`, or anything under the IDE/OS ignore globs.
