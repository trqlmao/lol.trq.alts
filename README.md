...
- **Async caches.** A small `AsyncCache<K,V>` primitive (lazy, non-blocking, stale-while-revalidate) powers player-head avatars (keyed by UUID, sourced from Mojang by default so no third party sees your account list) and optional, server-agnostic game stats. A host registers a `GameStatsSource` per server and the card renders whatever stat chips it returns; the library never interprets them.
- **Full account management.** `accountServices(account)` reads the whole profile (skins, capes, pending actions), checks entitlements as a product set (Java outright vs a lapsing Game Pass, Bedrock, …), and changes the name, skin, and cape — including a scheduled name claim that fires a burst around a drop time. All over the account's token; none of it installs a session.
